...
async function main() {
  ...
  try {
    ...
    const factory = await ethers.getContractFactory("WeightedPoolFactory2");

    const vault = newFileContents["Vault"];
    const protocolFeePercentagesProvider =
      newFileContents["ProtocolFeePercentagesProvider"];
    ...
    newFileContents["WeightedPoolFactory2"] = ret.address;
    historiesFileContents[`WeightedPoolFactory2-${timestamp}`] = ret.address;
