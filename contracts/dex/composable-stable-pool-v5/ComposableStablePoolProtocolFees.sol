    ...
    ProtocolFeeCache
{
    ...
    function _getProtocolPoolOwnershipPercentage(
        ...
        uint256 protocolYieldPercentage = nonExemptYieldGrowthInvariantDelta
            .divDown(totalGrowthInvariant)
            .mulDown(getProtocolFeePercentageCache(ProtocolFeeType.YIELD));
        ...
        )
    {
        ...
        swapFeeGrowthInvariant = Math.min(
            totalGrowthInvariant,
            swapFeeGrowthInvariant
        ); // Set upper bound.
        swapFeeGrowthInvariant = Math.max(
            lastPostJoinExitInvariant,
            swapFeeGrowthInvariant
        ); // Set lower bound.
    ...
    function _updateInvariantAfterJoinExit(
        ...
        if (postJoinExitInvariant > feelessInvariant) {
            uint256 invariantDeltaFromFees = postJoinExitInvariant -
                feelessInvariant;
            ...
            if (protocolOwnershipPercentage > 0) {
                uint256 protocolFeeAmount = ProtocolFees
                    .bptForPoolOwnershipPercentage(
                        postJoinExitSupply,
