...
abstract contract ComposableStablePoolRates is ComposableStablePoolStorage {
    ...
    constructor(RatesParams memory rateParams) {
        ...
        IERC20[] memory registeredTokens = _insertSorted(
            rateParams.tokens,
            IERC20(this)
        );
        uint256 bptIndex;
    ...
    function setTokenRateCacheDuration(
        IERC20 token,
        uint256 duration
    ) external authenticate whenNotInVaultContext {
    ...
    function _getAdjustedBalances(
        ...
        uint256[] memory adjustedBalances = new uint256[](
            totalTokensWithoutBpt
        );
    ...
    function _adjustedBalance(
        ...
        return
            Math.divDown(
                Math.mul(balance, cache.getOldRate()),
                cache.getCurrentRate()
            );
    }
