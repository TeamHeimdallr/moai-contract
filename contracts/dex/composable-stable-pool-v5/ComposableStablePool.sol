...
import "./v2-interfaces/contracts/pool-stable/StablePoolUserData.sol";
...
import "./v2-interfaces/contracts/pool-utils/IVersion.sol";
    ...
    ComposableStablePoolProtocolFees
{
    ...
    function _extractRatesParams(
        NewPoolParams memory params
    ) private pure returns (ComposableStablePoolRates.RatesParams memory) {
        return
    ...
    // Translate parameters to avoid stack-too-deep issues in the constructor
    function _extractStorageParams(
        NewPoolParams memory params
    ) private view returns (ComposableStablePoolStorage.StorageParams memory) {
        return
            ComposableStablePoolStorage.StorageParams({
                ...
                exemptFromYieldProtocolFeeFlag: params
    ...
    function _swapGivenIn(
        ...
        return
            (swapRequest.tokenIn == IERC20(this) ||
                swapRequest.tokenOut == IERC20(this))
                ? _swapWithBpt(
                    swapRequest,
                    registeredBalances,
                    registeredIndexIn,
    ...
    function _swapWithBpt(
        ...
        ) = registeredIndexOut == getBptIndex()
                ? _doJoinSwap(
                    isGivenIn,
                    swapRequest.amount,
                    balances,
    ...
    function _onJoinPool(
        ...
        return
            _onJoinExitPool(true, registeredBalances, scalingFactors, userData);
    }
    ...
    function _onJoinExitPool(
        ...
        function(uint256, uint256)
            internal
            pure
            returns (uint256) _addOrSub = isJoin
                ? FixedPoint.add
                : FixedPoint.sub;
    ...
    function _beforeJoinExit(
        uint256[] memory registeredBalances
    ) internal returns (uint256, uint256[] memory, uint256, uint256) {
        (
    ...
    function _doJoin(
        ...
        if (kind == StablePoolUserData.JoinKind.EXACT_TOKENS_IN_FOR_BPT_OUT) {
            ...
            kind == StablePoolUserData.JoinKind.ALL_TOKENS_IN_FOR_EXACT_BPT_OUT
        ) {
            return
    ...
    function _joinAllTokensInForExactBptOut(
        ...
        uint256[] memory amountsIn = StableMath._computeProportionalAmountsIn(
            balances,
            bptAmountOut,
            actualSupply
        );
    ...
    function _doExit(
        ...
        if (kind == StablePoolUserData.ExitKind.BPT_IN_FOR_EXACT_TOKENS_OUT) {
            ...
            kind == StablePoolUserData.ExitKind.EXACT_BPT_IN_FOR_ALL_TOKENS_OUT
        ) {
            return
                _exitExactBPTInForTokensOut(
                    preJoinExitSupply,
                    balances,
                    userData
                );
    ...
    function _doRecoveryModeExit(
        ...
        (
            ...
            uint256[] memory balances
        ) = _dropBptItemFromBalances(registeredBalances);

        (uint256 bptAmountIn, uint256[] memory amountsOut) = super
            ._doRecoveryModeExit(balances, virtualSupply, userData);
        ...
        )
    {
        ...
        (, uint256[] memory registeredBalances, ) = getVault().getPoolTokens(
            getPoolId()
        );
        _upscaleArray(registeredBalances, _scalingFactors());
        (virtualSupply, balances) = _dropBptItemFromBalances(
            registeredBalances
        );
    ...
    function _onDisableRecoveryMode() internal override whenNotInVaultContext {
        ...
        (uint256 currentAmp, ) = _getAmplificationParameter();
        uint256 currentInvariant = StableMath._calculateInvariant(
            currentAmp,
            balances
        );
