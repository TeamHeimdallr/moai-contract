...
import "./v2-interfaces/contracts/solidity-utils/openzeppelin/IERC20.sol";
...
abstract contract ComposableStablePoolStorage is BasePool {
    ...
    constructor(StorageParams memory params) {
        ...
        _scalingFactor3 = totalTokens > 3
            ? _computeScalingFactor(params.registeredTokens[3])
            : 0;
        _scalingFactor4 = totalTokens > 4
            ? _computeScalingFactor(params.registeredTokens[4])
            : 0;
        _scalingFactor5 = totalTokens > 5
            ? _computeScalingFactor(params.registeredTokens[5])
            : 0;
        ...
        IRateProvider[] memory rateProviders = new IRateProvider[](
            params.registeredTokens.length
        );
        ...
        for (uint256 i = 0; i < params.registeredTokens.length; ++i) {
            ...
            if (
                params.exemptFromYieldProtocolFeeFlag &&
                rateProviders[i] != IRateProvider(0)
            ) {
                rateProviderInfoBitmap = rateProviderInfoBitmap.insertBool(
                    true,
        ...
        _rateProvider3 = (rateProviders.length > 3)
            ? rateProviders[3]
            : IRateProvider(0);
        _rateProvider4 = (rateProviders.length > 4)
            ? rateProviders[4]
            : IRateProvider(0);
        _rateProvider5 = (rateProviders.length > 5)
            ? rateProviders[5]
            : IRateProvider(0);
    ...
    function _addBptIndex(
        uint256 index
    ) internal view returns (uint256 registeredIndex) {
        ...
        _require(
            registeredIndex < _totalTokens && registeredIndex != getBptIndex(),
    ...
    function _addBptItem(
        ...
        for (uint256 i = 0; i < registeredTokenAmounts.length; i++) {
            registeredTokenAmounts[i] = i == getBptIndex()
                ? bptAmount
                : amounts[i < getBptIndex() ? i : i - 1];
        }
    }
    ...
    function _hasRateProvider(uint256 tokenIndex) internal view returns (bool) {
        return
            _rateProviderInfoBitmap.decodeBool(
                _RATE_PROVIDER_FLAGS_OFFSET + tokenIndex
            );
    }
    ...
    function isTokenExemptFromYieldProtocolFee(
        ...
        return
            _exemptFromYieldProtocolFee &&
            _hasRateProvider(_getTokenIndex(token));
    }
